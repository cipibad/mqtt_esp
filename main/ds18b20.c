...
esp_err_t ds18b20_get_temp(int16_t *temp) {
  if(init==1){
    unsigned char check;
    char temp1=0, temp2=0;
      ...
      if(check==1)
      {
        ...
        check=ds18b20_RST_PULSE();
        *temp = 10 * (temp1+(temp2*256))/16;
        return ESP_OK;
      }
      else {
        ESP_LOGE("ds18b20", "ESP_ERR_INVALID_RESPONSE");
        return ESP_ERR_INVALID_RESPONSE;
      }

  }
  else {
    ESP_LOGE("ds18b20", "ESP_ERR_INVALID_STATE");
    return ESP_ERR_INVALID_STATE;
    }
}
void ds18b20_init(int GPIO){
  DS_GPIO = GPIO;
  // porting gpio_pad_select_gpio(DS_GPIO);
  init=1;
}
