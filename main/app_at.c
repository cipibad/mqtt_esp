...
void AtInitTimerCallback( TimerHandle_t xTimer )
{
    ...
    LOGI(LOG_MODULE_SYSTEM, "timer %s expired", pcTimerName);
...
void app_at_task(void* pvParameters)
{
    ...
    LOGI(LOG_MODULE_SYSTEM, "UART AT server ready");
    ...
    if (atInitTimer == NULL) {
        LOGE(LOG_MODULE_SYSTEM, "Cannot create atInitTimer timer");
        return;
    }
    if (xTimerStart(atInitTimer, 0) != pdPASS) {
        LOGE(LOG_MODULE_SYSTEM, "Cannot start atInitTimer");
        return;
    }
    ...
    while (1) {
        ...
        if (len > 0) {
            data[len] = 0;
            LOGI(LOG_MODULE_SYSTEM, "Read: %d bytes: >%.*s<", len, len, data);
