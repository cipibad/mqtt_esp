...
void handle_udp_message(udp_msg_t* udp_msg)
{
    ...
    mqtt_publish_data(udp_msg->msg.topic, udp_msg->msg.data, udp_msg->msg.qos, udp_msg->msg.retain);
    return;
...
void udp_server_task(void *pvParameters)
{
	...
	// unsigned char key[] = {0x13, 0x59, 0x14, 0x8A, 0xC2, 0x11, 0x6D, 0x6C, 0x42, 0x06, 0xAE, 0x95, 0x4D, 0xE5, 0xFB, 0x00, 0x7A, 0x95, 0x15, 0xAA, 0xCF, 0x0D, 0x32, 0x12, 0x2C, 0xEA, 0xD1, 0x85, 0x38, 0xA0, 0x33, 0xB3};
    // for (int i = 0; i < 32; i++)
    // {
    //     printf("%02X, ", key[i]);
    // }
	...
	// ret = mbedtls_aes_crypt_cfb128(&aes_enc, MBEDTLS_AES_ENCRYPT, crypt_len, &iv_offset_enc, iv_enc, input, output);
	// if (ret != 0) {
	...
	// ret = mbedtls_aes_crypt_cfb128(&aes_dec, MBEDTLS_AES_DECRYPT, crypt_len, &iv_offset_dec, iv_dec, output, input);
	// if (ret != 0) {
